...
import { Event, STORAGE_KEYS } from './types.js';
import { Storage, EventStorage } from './storage.js';

/**
 * Parse a single history line into an Event object
 */
function parseHistoryLine(line: string, lineIndex: number): Event | null {
  if (!line.trim()) {
    return null;
  }
 ...
 */
export function migrateHistoryToEvents(historyText: string): Event[] {
  const events: Event[] = [];
 ...
 */
export function performMigration(): number | null {
  ...
  const historyText = Storage.load<string>(STORAGE_KEYS.HISTORY_V2, '');
  if (!historyText) {
 ...
 */
export function undoMigration(): boolean {
  const backup = Storage.load<string>('lifeos_pro_history_v2_backup', '');
  if (!backup) {
 ...
 */
export function canUndoMigration(): boolean {
  const migrationDate = Storage.load<string>('lifeos_pro_migration_date', '');
  if (!migrationDate) {
    return false;
  }
