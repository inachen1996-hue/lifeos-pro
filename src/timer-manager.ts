...
export class TimerManager {
  ...
  static updateTimer(
    ...
    syncExistingRecords: boolean = false
  ): Timer {
   ...
   */
  static deleteTimer(timerId: string, deleteRecords: boolean = false): void {
    const timers = TimerStorage.loadTimers();
    const timer = timers.find(t => t.id === timerId);
  ...
  static getTimersGroupedByCategory(): Map<TimerCategory, Timer[]> {
    ...
    const timers = TimerStorage.loadTimers();
    const grouped = new Map<TimerCategory, Timer[]>();
   ...
   */
  private static deleteTimerRecords(timerId: string): void {
    const events = EventStorage.loadEvents();
